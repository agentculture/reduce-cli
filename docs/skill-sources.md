...
reduce-cli vendors its `.claude/skills/` from **guildmaster** — the
...
Every vendored `SKILL.md` carries `type: command`. reduce-cli
...
#   - consumer-identifying prose: `guildmaster` → `reduce-cli` (NOT
...
If a re-sync would lose a reduce-cli adaptation, lift the change
...
#     → `which reduce-cli vendors from guildmaster`
