"""``reduce-cli learn`` — the learnability affordance.
...
from reduce.cli._output import emit_result

_TEXT = """\
reduce-cli — a clonable template for AgentCulture mesh agents.
...
Commands
--------
  reduce-cli whoami             Identity from culture.yaml.
  reduce-cli learn              This self-teaching prompt.
  reduce-cli explain <path>...  Markdown docs for any noun/verb path.
  reduce-cli overview           Descriptive snapshot of the agent.
  reduce-cli doctor             Check the agent-identity invariants.
  reduce-cli cli overview       Describe the CLI surface itself.
...
def _as_json_payload() -> dict[str, object]:
    return {
        ...
        "json_support": True,
        "explain_pointer": "reduce-cli explain <path>",
    }
