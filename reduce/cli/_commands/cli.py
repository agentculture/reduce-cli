"""``reduce-cli cli`` — noun grouping CLI-surface introspection.
...
from reduce.cli._commands.overview import cli_sections, emit_overview
...
def _no_verb(args: argparse.Namespace) -> int:
    # `reduce-cli cli` with no sub-verb prints the noun's overview.
...
def register(sub: argparse._SubParsersAction) -> None:
    p = sub.add_parser(
        "cli",
        help="CLI-surface introspection (see 'reduce-cli cli overview').",
    )
    ...
    ov = noun_sub.add_parser("overview", help="Describe the reduce-cli CLI surface.")
