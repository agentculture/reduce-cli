...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope reduce-cli
...
scope:** the default personal scope is **private** (`--scope reduce-cli
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `reduce-cli`). Private records are served only to a recall in the same
