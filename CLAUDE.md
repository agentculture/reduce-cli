...
This repository hosts the **reduce-cli** agent.
...
Until you run `/init`, `reduce-cli` satisfies the `steward doctor`
...
`reduce-cli` scope), so memory travels with the repo, not a private
